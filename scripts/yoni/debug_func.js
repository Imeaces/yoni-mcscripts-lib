...
(async function(){
    ...
    doEval = new Function(`
        ...
        EventTypes, events, Logger, Command, ChatCommand, send, say, YoniEntity, sv,
        setTimeout, clearTimeout, setInterval, clearInterval } = arguments[0];
...
function openShell(sender){
    ...
    gui.show(YoniEntity.getMinecraftEntity(sender))
    .then(rt => {
        logger.info('has rt');
        if (!("canceled" in rt)){
