...
export function getNumArr(num, base = 10, numbers = addressNumbers) {
    ...
    for (let n of num) {
        ...
        if (idx >= base)
            throw new Error("number base too big");
        if (idx === -1)
            throw new Error("cover " + n + " failed");
        arr.push(idx);
    }
    if (fs) {
        arr.unshift("-");
...
export function getNum(numArr, base = 10, numbers = addressNumbers) {
    ...
    for (let idx = 0; idx < numArr.length; idx++) {
        let n = numArr[idx];
        if (idx === 0 && idx !== (numArr.length - 1) && n === 0)
            throw new Error("zero before number array");
        if (!isFinite(n))
            throw new Error("number not valid " + n);
        if (base <= n)
            throw new Error("number " + n + " bugger than base " + base);
        num += numbers[n];
    }
...
export function splitNumberArr(numArr, sp, base = 10) {
    ...
    for (let i = 0; i < numArr.length; i++) {
        if (!isFinite(numArr[i]))
            throw new Error("number not valid " + numArr[i]);
