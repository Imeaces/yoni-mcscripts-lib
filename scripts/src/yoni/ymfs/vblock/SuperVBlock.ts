import { VBlock } from "./VBlock.js";
import { VBlockTypes } from "./VBlockTypes.js";
import { VBlockPosition } from "./VBlockPosition.js";
...
是否需要存储已使用的区块？
 */
export class SuperVBlock extends VBlock {
    static ymfsHead = ["Yoni_YmFS", 1];

    fsinfo: YmFS_FSInfo;
    spblocks: VBlockPosition[];
    /**
     * 为指定块设置状态，并更新记录。
     *
     * 将状态设置为被占用类型时，不会从可用块记录移除特定块。
     */
    setStat(index: number, value: VBlockTypes){
        switch (value){
            case VBlockTypes.DeletedVBlock:
                this.deletedChunks.add(index);
            case VBlockTypes.EmptyVBlock:
                this.availableChunks.push(index);
                this.occupiedChunks.delete(index);
                break;
            case VBlockTypes.DataVBlock:
            case VBlockTypes.NormalVBlock:
            case VBlockTypes.SuperVBlock:
