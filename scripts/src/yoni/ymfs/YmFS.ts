...
class YmFSSuperChunk extends YmFSChunk {
    static #YmFS_FSInfo = class YmFS_FSInfo {
        ...
        set rootDirectoryChunkIndex(index): number {
    ...
    constructor(chunk: Chunk){
        ...
        this.spblock = this.getDataStr(1)
            .split("\u0000")
            .map(Number);
        
        this.fsinfo = new YmFSSuperChunk.YmFS_FSInfo(this);
...
class YmFS_FileNode {
    ...
    static readStringArray(block: Block): string[] {
        let str = block.getDataStr();
        return str.split("\u0000");
    }
    static writeStringArray(block: Block, arr: any[]){
