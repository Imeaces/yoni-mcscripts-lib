...
class Dimension {
    ...
    getEntities(options?: Partial<Minecraft.EntityQueryOptions>): YoniEntity[] {
        return Array.from(this.vanillaDimension.getEntities(Object.assign(new Minecraft.EntityQueryOptions, options))).map(EntityUtils.from) as unknown as YoniEntity[];
    }
    getEntitiesAtBlockLocation(location: Vector3): YoniEntity[] {
        return Array.from(this.vanillaDimension.getEntitiesAtBlockLocation(new Location(location).getVanillaBlockLocation()))
    ...
    getEntitiesFromRay(location: Vector3, direction: Vector3, options?: Minecraft.EntityRaycastOptions): YoniEntity[] {
        return Array.from(this.vanillaDimension.getEntitiesFromRay(new Location(location).getVanillaLocation(), new Location(direction).getVanillaVector(), options))
            .map(EntityUtils.from) as unknown as YoniEntity[];
    }
    getPlayers(option?: Partial<Minecraft.EntityQueryOptions>): YoniPlayer[] {
        return Array.from(this.vanillaDimension.getPlayers(Object.assign(new Minecraft.EntityQueryOptions, option)))
            .map(EntityUtils.from) as unknown as YoniPlayer[];
    }
    ...
    spawnEntity(identifier: string, location: Vector3): YoniEntity {
        return EntityUtils.from(this.vanillaDimension.spawnEntity(identifier, new Location(location).getVanillaLocation())) as YoniEntity;
    }
    spawnItem(item: Minecraft.ItemStack, location: Vector3): YoniEntity {
        return EntityUtils.from(this.vanillaDimension.spawnItem(item, new Location(location).getVanillaLocation())) as YoniEntity;
    }
}
